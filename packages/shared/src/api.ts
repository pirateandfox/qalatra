// Qalatra HTTP API client — every backend endpoint, platform-agnostic.
//
// Ported verbatim from the desktop UI's api.ts. The only platform-specific
// function, openAttachmentFile (which used window.open), is intentionally left to
// the host; this module exposes the portable fetchAttachmentBlob primitive
// instead, and the host decides how to present the blob.
...
import { currentServerInstance, enc, httpJson, jsonRequest, v1 } from './runtime'
...
/**
 * Fetch an attachment's raw bytes as a Blob. The host decides how to present it
 * (the desktop UI opens it in a new window via window.open; mobile would hand it
 * to a share/preview sheet).
 */
